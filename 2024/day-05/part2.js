...
function main() {
    ...
    inputUpdates.forEach(update => {
        if (!followsAllRules(update)) {
    ...
    incorrectUpdates.forEach(update => {
        fixedUpdates.push(fixUpdate(update))
    })

    let middlePageNumberCount = 0
    fixedUpdates.forEach(update => {
        middlePageNumberCount += Number(findMiddleValue(update.split(",")))
    })

    console.log(middlePageNumberCount)
}

main() // 4145

function followsAllRules(update) {
    let failsRule = true
...
function followsRule(update, rule) {
    valuesInRule = rule.split("|")
    regex = new RegExp(`${valuesInRule[1]}.*${valuesInRule[0]}`)
    return !regex.test(update)
}

function findMiddleValue(update) {
...
function fixUpdate(update) {
    ...
    let isFixed = false
    while (!isFixed) {
        if (followsAllRules(fixedUpdate)) {
            isFixed = true
            break
        }
        const ruleToFix = findNextFailingRule(fixedUpdate)
        const iteration = fixUpdateForRule(fixedUpdate, ruleToFix)
        console.log(`${fixedUpdate}, ${iteration}, ${ruleToFix}`)
...
function findNextFailingRule(update) {
    let failsRule = undefined
    inputRules.forEach(rule => {
        if (!followsRule(update, rule)) {
            failsRule = rule
        }
...
function fixUpdateForRule(update, rule) {
    valuesInRule = rule.split("|")
    return update.replace(valuesInRule[1], "aa").replace(valuesInRule[0], valuesInRule[1]).replace("aa", valuesInRule[0])
}

// console.log(fixUpdateForRule("75,97,47,61,53", "97|75"))