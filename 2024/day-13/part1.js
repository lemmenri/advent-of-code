...
function main() {
    ...
    data.forEach(formula => {
        const price = calculateMinimunPrice(formula)
        totalPrice += price !== undefined ? price : 0
        console.log(price)
    })
...
function parseInput(rawData) {
    ...
    for (let i = 0; i < rawData.length; i += 4) {
        ...
        const ay = rawData[i].split(":")[1].trim().split(" ")[1].split("+")[1]
        let bx = rawData[i + 1].split(":")[1].trim().split(" ")[0].split("+")[1]
        bx = bx.substring(0, bx.length - 1)
        const by = rawData[i + 1].split(":")[1].trim().split(" ")[1].split("+")[1]
        let prizex = rawData[i + 2].split(":")[1].trim().split(" ")[0].split("=")[1]
        prizex = prizex.substring(0, prizex.length - 1)
        const prizey = rawData[i + 2].split(":")[1].trim().split(" ")[1].split("=")[1]
        data.push({
            a: {
                x: ax,
                y: ay,
