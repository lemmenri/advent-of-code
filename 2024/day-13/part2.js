...
function main() {
    ...
    console.log(data)
    // const solution = solveSystem(data[0])
    let totalPrice = 0
    data.forEach(formula => {
        const price = solveSystem(formula)
        totalPrice += price !== undefined ? (3 * price.a + price.b) : 0
        console.log(price)
    })
    console.log(totalPrice)
}

main() // 108713182988244
...
//     [34, 67, 5400]
// ]
// console.log(solveSystem(matrix))

function calculateMinimunPrice(formula) {
    const map = createMapOfPrizes(formula)
    const target = JSON.stringify({ x: Number(formula.prize.x), y: Number(formula.prize.y) })
    return map.get(target)
}

function createMapOfPrizes(formula) {
    const maxNrOfAPresses = 100, maxNrOfBPresses = 100
    const map = new Map()

    for (let i = maxNrOfAPresses; i >= 0; i--) {
        for (let j = 0; j <= maxNrOfBPresses; j++) {
            const prizePosition = calculateEndPosition(formula, i, j)
            map.set(JSON.stringify({ x: prizePosition.x, y: prizePosition.y }), (3 * i + j))
        }
    }
...
function calculateEndPosition(formula, nrOfAPresses, nrOfBPresses) {
    let x = 0, y = 0
    x += (Number(formula.a.x) * nrOfAPresses)
    y += (Number(formula.a.y) * nrOfAPresses)
    x += (Number(formula.b.x) * nrOfBPresses)
    y += (Number(formula.b.y) * nrOfBPresses)
...
//     prize: { x: '8400,', y: '5400' }
// }
// console.log(calculateEndPosition(formula, 80, 40)) // 8400, 5400
...
function parseInput(rawData) {
    ...
    for (let i = 0; i < rawData.length; i += 4) {
        let ax = rawData[i].split(":")[1].trim().split(" ")[0].split("+")[1]
        ax = Number(ax.substring(0, ax.length - 1))
        const ay = Number(rawData[i].split(":")[1].trim().split(" ")[1].split("+")[1])
        let bx = rawData[i + 1].split(":")[1].trim().split(" ")[0].split("+")[1]
        bx = Number(bx.substring(0, bx.length - 1))
        const by = Number(rawData[i + 1].split(":")[1].trim().split(" ")[1].split("+")[1])
        let prizex = rawData[i + 2].split(":")[1].trim().split(" ")[0].split("=")[1]
        prizex = Number(prizex.substring(0, prizex.length - 1))
        prizex += offset
        let prizey = Number(rawData[i + 2].split(":")[1].trim().split(" ")[1].split("=")[1])
        prizey += offset
        data.push([
            [ax, bx, prizex],
            [ay, by, prizey]
