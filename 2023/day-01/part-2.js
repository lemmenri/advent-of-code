...
function findFirstDigit(string) {
    return stringToInt(string.match(/(one|two|three|four|five|six|seven|eight|nine|1|2|3|4|5|6|7|8|9)/)[0])
}
...
function findLastDigit(string) {
    ...
    digits.forEach(digit => {
        const lastIndex = string.lastIndexOf(digit)
        if (lastIndex > latestIndex) {
            latestDigit = digit
            latestIndex = lastIndex
        }
    })
    return stringToInt(latestDigit)
}

let sum = 0;
input.forEach(row => {
    const firstInt = findFirstDigit(row)
    const lastInt = findLastDigit(row)
    const rowValue = firstInt * 10 + (lastInt * 1 % 10)
    console.log(firstInt + " " + lastInt + " " + rowValue)
    if (rowValue > 100) {
