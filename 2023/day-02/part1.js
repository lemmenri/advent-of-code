...
function compareToMax(input) {
    const data = input.trim().split(" ")
    if (data[1] === "red") { return data[0] <= maxRed }
    if (data[1] === "green") { return data[0] <= maxGreen }
    if (data[1] === "blue") { return data[0] <= maxBlue }
}
...
input.forEach((game, index) => {
    ...
    const data = []
    hands.forEach(hand => {
        data.push(hand.split(",").map(function (item) {
            // console.log(`Comparing '${item}' resulting in '${compareToMax(item)}'`)
            if (compareToMax(item) === false) {
                isValidGame = false
            }
            return item.trim();
        }))
    ...
    if (isValidGame) { sumOfValidGameIds += index + 1 }
})
console.log(`Sum: ${sumOfValidGameIds}`) //3035