...
function getPower(stones) {
    return Number(stones.red) * Number(stones.blue) * Number(stones.green)
}
...
function getMinimumCubes(games) {
    ...
    games.forEach(game => {
        game.split(",").map(function (item) {
            const data = item.trim().split(" ")
            if (data[1] === "red" && Number(data[0]) > stones.red) { stones = { ...stones, red: Number(data[0]) } }
            if (data[1] === "green" && Number(data[0]) > stones.green) { stones = { ...stones, green: Number(data[0]) } }
            if (data[1] === "blue" && Number(data[0]) > stones.blue) { stones = { ...stones, blue: Number(data[0]) } }
        })
    })
...
// console.log(getPower(getMinimumCubes(["1 green, 3 red, 6 blue", " 3 green, 6 red", " 5 green, 10 blue, 14 red"])))

let sumOfPowers = 0
input.forEach((game) => {
    const power = getPower(getMinimumCubes(game.split(":")[1].split(";")))
    // console.log(power)
    sumOfPowers += power
})

console.log("Sum: " + sumOfPowers) // 66027