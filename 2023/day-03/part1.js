...
dataArray.forEach((row, i) => {
    row.forEach((cell, j) => {
        ...
        const neighbours = []
        i > 0 && j > 0 && neighbours.push(dataArray[i - 1][j - 1])
        j > 0 && neighbours.push(dataArray[i][j - 1])
        i < (dataArray.length - 1) && j > 0 && neighbours.push(dataArray[i + 1][j - 1])
        for (let k = 0; k < numberLength; k++) {
            i > 0 && neighbours.push(dataArray[i - 1][j + k])
            i < (dataArray.length - 1) && neighbours.push(dataArray[i + 1][j + k])
        }
        i > 0 && (j + numberLength) < (dataArray[0].length - 1) && neighbours.push(dataArray[i - 1][j + numberLength]);
        ((j + numberLength) < (dataArray[0].length - 1)) && neighbours.push(dataArray[i][j + numberLength]);
        i < (dataArray.length - 1) && (j + numberLength) < (dataArray[0].length - 1) && neighbours.push(dataArray[i + 1][j + numberLength]);

        console.log(neighbours)

        neighbours.forEach(neighbour => {
            if (containsSpecialCharacter(neighbour)) {
...
const sum = listOfValidNumbers.reduce((partialSum, a) => partialSum + a, 0);
console.log(sum); // 520019