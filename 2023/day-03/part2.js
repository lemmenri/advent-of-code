...
const dataArray = []
input.forEach(row => { // split input in 2 dimensional array
...
function containsGear(str) {
    return str === '*'
}

const listOfValidNumbers = []
const listOfNumbersWithGears = []

let skipNrOfNextCells = 0

dataArray.forEach((row, i) => {
    row.forEach((cell, j) => {
        if (skipNrOfNextCells > 0) {
            skipNrOfNextCells--;
            return
        }
        if (!isNumeric(cell)) {
        ...
        // console.log('found a number: ' + cell)
        let currentNumber = Number(cell)

        let nextCellIsANumber = true
        let n = 1
        while (nextCellIsANumber) {
            const nextCell = dataArray[i][j + n]
            if (!isNumeric(nextCell)) {
                // console.log('next cell is not a number')
                nextCellIsANumber = false
            } else {
                // console.log('Next cell is also a number:' + nextCell)
                currentNumber = currentNumber * 10 + Number(nextCell)
                skipNrOfNextCells++
                n++
            }
        }
        console.log('Current number: ' + currentNumber)
        // TODO: check if currentNumber has any special characters surrounding it
        const numberLength = currentNumber.toString().length
        const neighbours = []
        i > 0 && j > 0 && neighbours.push({ char: dataArray[i - 1][j - 1], y: i - 1, x: j - 1 })
        j > 0 && neighbours.push({ char: dataArray[i][j - 1], y: i, x: j - 1 })
        i < (dataArray.length - 1) && j > 0 && neighbours.push({ char: dataArray[i + 1][j - 1], y: i + 1, x: j - 1 })
        for (let k = 0; k < numberLength; k++) {
            i > 0 && neighbours.push({ char: dataArray[i - 1][j + k], y: i - 1, x: j + k })
            i < (dataArray.length - 1) && neighbours.push({ char: dataArray[i + 1][j + k], y: i + 1, x: j + k })
        }
        i > 0 && (j + numberLength) < (dataArray[0].length - 1) && neighbours.push({ char: dataArray[i - 1][j + numberLength], y: i - 1, x: j + numberLength });
        ((j + numberLength) < (dataArray[0].length - 1)) && neighbours.push({ char: dataArray[i][j + numberLength], y: i, x: j + numberLength });
        i < (dataArray.length - 1) && (j + numberLength) < (dataArray[0].length - 1) && neighbours.push({ char: dataArray[i + 1][j + numberLength], y: i + 1, x: j + numberLength });

        console.log(neighbours)

        neighbours.forEach(neighbour => {
            if (containsSpecialCharacter(neighbour.char)) {
                if (containsGear(neighbour.char)) {
                    listOfNumbersWithGears.push({ number: currentNumber, x: neighbour.x, y: neighbour.y })
                    console.log('found a gear!')
                } else {
                    listOfValidNumbers.push(currentNumber);
...
console.log(listOfValidNumbers)
console.log(listOfNumbersWithGears)

let sum = 0

listOfNumbersWithGears.forEach((asterix, index) => {
    const partialSum = isGearWith(asterix, listOfNumbersWithGears.slice(index + 1))
    if (partialSum !== false) {
        sum += partialSum
    }
...
// return false if not a gear
function isGearWith({ number, x, y }, restOfList) {
    const otherNumber = restOfList.find((element) => element.x === x && element.y === y)
    if (otherNumber !== undefined) { return number * otherNumber.number }
    return false
}


console.log(sum); // 75519888